...
class WebSolver:
    ...
    def _reload_service(self):
        ...
        # Tier 2: Kernel fallback (optional — silently disabled if SDK missing or key absent)
        try:
            from modules.services.fallback_solver import FallbackSolver
            candidate = FallbackSolver(self.bot)
            self.fallback = candidate if candidate.api_key else None
        except Exception:
            self.fallback = None
    ...
    async def auto_verify(self, tries=3):
        ...
        self.bot.log("WARN", "BrowserUse: Full flow failed.")

        # Tier 2: Kernel fallback
        if not getattr(self, 'fallback', None):
            self.bot.log("WARN", "Fallback: Kernel fallback unavailable (missing KERNEL_API_KEY or SDK).")
            return False

        self.bot.log("SYS", "Fallback: Attempting Kernel flow...")
        fallback_success = await self.fallback.solve_full_flow(self.bot.token, retries=2)
        if fallback_success:
            self.mark_verification_done(str(self.bot.user.id))
            self.bot.log("SUCCESS", "Fallback: Captcha solved and verified via Kernel!")
            return True

        self.bot.log("ERROR", "Fallback: Kernel flow failed.")
        return False
