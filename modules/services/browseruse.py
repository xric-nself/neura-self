...
class BrowserUseService:
    ...
    # Hard cap on total time spent in solve_full_flow. If exceeded,
    # abort and let the Kernel fallback take over.
    TIME_BUDGET_SECONDS = int(os.getenv("BROWSERUSE_TIME_BUDGET", "420"))
    ...
    async def solve_full_flow(self, discord_token, retries=3):
        ...
        for attempt in range(retries):
            ...
            if elapsed > self.TIME_BUDGET_SECONDS:
                self.bot.log(
                    "WARN",
                    "BrowserUse: Time budget exceeded before attempt "
                    + str(attempt + 1) + " (" + str(int(elapsed)) + "s > "
                    + str(self.TIME_BUDGET_SECONDS) + "s). Aborting to fallback.",
                )
                aborted = True
                break
            ...
            try:
                ...
                async with async_playwright() as p:
                    ...
                    for _ in range(55):
                        ...
                        if time.monotonic() - start_time > self.TIME_BUDGET_SECONDS:
                            self.bot.log(
                                "WARN",
                                "BrowserUse: Time budget exceeded mid-poll. Breaking attempt.",
                            )
                            aborted = True
                            break
                ...
                if session_id:
                    await self._stop_browser(session_id)
                if aborted:
                    # Timer expired — do not sleep, break out immediately.
                    break
                if attempt < retries - 1:
                    await asyncio.sleep(5)

        if aborted:
            self.bot.log("WARN", "BrowserUse: Aborted by time budget — handing off to Kernel.")
        return False
