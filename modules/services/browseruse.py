...
class BrowserUseService:
    ...
    # Rotate through these proxy countries if an attempt fails.
    ...
    async def _cleanup_old_sessions(self):
        """Stop orphaned sessions older than CLEANUP_AGE_MINUTES."""
        if not self.api_key:
            return
    ...
    async def _create_browser(self, proxy_country="us"):
        """Create a browser session with the given proxy country."""
        async with aiohttp.ClientSession() as session:
    ...
    async def _stop_browser(self, session_id):
        """Stop a browser session. Official v4 endpoint: PATCH /browsers/{id} with {"action":"stop"}"""
        try:
            async with aiohttp.ClientSession() as session:
    ...
    async def solve_full_flow(self, discord_token, retries=3):
        ...
        for attempt in range(retries):
            ...
            # Rotate proxy country on each attempt
            country = self.PROXY_COUNTRIES[attempt % len(self.PROXY_COUNTRIES)]
