...
class NeuraGems(commands.Cog):
    def __init__(self, bot):
        ...
        # Pending inventory window: while set, any reply containing
        # "'s inventory" or "'s gems" is accepted as our response, even if
        # another command was sent in the meantime.
        # Matched to core/bot.py's 20s checking_gems timeout.
        self.pending_inv = 0.0
    ...
    async def on_message(self, message):
        ...
        # NEW: Pending-inventory window. If we sent `owo inv` recently, accept
        # any reply that looks like an inventory response — even if another
        # command was sent in between, which breaks the last_sent_command check.
        if not is_for_me and time.time() < self.pending_inv:
            if "'s inventory" in content or "'s gems" in content:
                is_for_me = True
             ...
             if state.checking_gems.get(self.bot.user_id):
                if not self.bot.is_message_for_me(message, role="header"):
                    if not self.bot.is_message_for_me(message):
                        # NEW: if we're inside the pending inventory window, accept anyway.
                        if time.time() >= self.pending_inv:
                            return
